...
// We connect to a separately-hosted relay (relay/server.js, deployed on its
// own always-on host), which holds the Sarvam key and adds it as a header
// upstream. Browsers can't set WS headers, and Sarvam rejects query-param
// auth — hence the relay. Since the relay is a different origin from the app
// (not same-origin anymore), we authenticate with a live Firebase ID token
// instead of relying on same-origin as an implicit trust boundary.
async function buildRelayUrl(sessionId: string, speaker: string): Promise<string | null> {
  const relayBase = process.env.NEXT_PUBLIC_STT_RELAY_URL
  if (!relayBase) {
    console.error('[Transcription] NEXT_PUBLIC_STT_RELAY_URL is not configured')
    return null
  }
  ...
  return `${relayBase}/api/sarvam-stream?${params.toString()}`
}
...
export function useSessionTranscription({
  ...
  const startPipe = useCallback(
    async (key: string, track: MediaStreamTrack, speaker: TranscriptChunk['speaker']) => {
      ...
      // Reserve the slot before the async token fetch so a second call for the
      // same key (e.g. re-fired TrackSubscribed) can't race in and double-open.
      pipesRef.current.set(key, null as unknown as Pipe)

      const relayUrl = await buildRelayUrl(sessionId, speaker)
      // stopPipe() may have run while we were awaiting the token (e.g. the
      // participant left, or the session ended) — bail out rather than
      // opening a socket for a pipe nobody is tracking anymore.
      if (!pipesRef.current.has(key)) return
      if (!relayUrl) {
        pipesRef.current.delete(key)
        setState((s) => ({ ...s, error: 'Transcription relay is not configured' }))
        return
      }

      // Connect to the relay (translate mode → English out, multilingual in).
