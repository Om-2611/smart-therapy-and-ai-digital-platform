    ...
    <div style={{ display: 'flex', gap: 2, background: '#F3F4F8', borderRadius: 999, padding: 3 }}>
      {options.map(o => {
        ...
        return (
          ...
          >
            {o.icon && <span aria-hidden style={{ fontSize: 12 }}>{o.icon}</span>}
            {o.label}
          </button>
        )
      })}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ ...card, gap: 0, padding: '7px 6px' }}>
          <div style={{ padding: '0 14px', textAlign: 'center', minWidth: 74 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: VIOLET }}>Round</div>
            <div style={{ fontSize: 16, fontWeight: 800, color: INK, lineHeight: 1.25 }}>{roundLabel}</div>
          </div>
          <div style={{ width: 1, alignSelf: 'stretch', background: CARD_BORDER }} />
          <div style={{ padding: '0 14px', textAlign: 'center', minWidth: 66 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: VIOLET, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
              <span aria-hidden>⭐</span> Score
            </div>
            <div style={{ fontSize: 16, fontWeight: 800, color: INK, lineHeight: 1.25 }}>{score}</div>
          </div>
        </div>
          ...
          <div style={{ ...card, gap: 8 }}>
            ...
            {!isPlaying ? (
              <button type="button" onClick={handleStart} style={{
                ...
                fontSize: 12, fontWeight: 800, boxShadow: '0 3px 10px rgba(91,33,182,0.32)',
              }}>▶ Start</button>
            ) : (
              <button type="button" onClick={handlePause} style={{
                ...
                background: '#F3F4F8', color: INK, fontSize: 12, fontWeight: 800,
              }}>⏸ Pause</button>
            )}
            <button type="button" onClick={handleReset} style={{
              padding: '7px 13px', borderRadius: 999, border: '1px solid rgba(225,29,72,0.35)', cursor: 'pointer',
              background: 'transparent', color: '#BE123C', fontSize: 12, fontWeight: 700,
            }}>↺ Reset</button>
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', alignItems: 'stretch', gap: 14 }}>
        ...
        <div style={{ flexShrink: 0, width: 118, display: 'flex', flexDirection: 'column', gap: 12, minHeight: 0 }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div aria-hidden style={{
                ...
                bottom: `${levelPct}%`, fontSize: 13, lineHeight: 1,
                transition: 'bottom 0.45s cubic-bezier(.4,0,.2,1)',
              }}>⭐</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: INK_MUTED }}>Level</div>
              <div style={{ fontSize: 14, fontWeight: 800, color: INK, lineHeight: 1.2 }}>{levelValue}</div>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 12, fontWeight: 800, color: VIOLET }}>Lives</div>
            ...
            <div style={{ width: '100%', maxWidth: 620, height: '100%', display: 'flex', flexDirection: 'column', gap: 12, justifyContent: 'center' }}>
              <div key={animateKey} className="ci-a" style={{
                ...
                {currentCmd?.hasSimonSays ? (
                  <div style={{ fontSize: 12, fontWeight: 800, color: VIOLET, letterSpacing: 0.3 }}>SIMON SAYS...</div>
                ) : (
                  <div style={{ fontSize: 12, fontWeight: 800, color: 'transparent' }}>&nbsp;</div>
                )}
                <div style={{ fontSize: 26, fontWeight: 800, color: INK, textAlign: 'center', lineHeight: 1.25 }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 17, fontWeight: 800, color: VIOLET, lineHeight: 1.25 }}>{banner.title}</div>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#4A4560', lineHeight: 1.35 }}>{banner.sub}</div>
        </div>
        ...
        <button
          ...
          style={{
            ...
            fontSize: 14, lineHeight: 1, color: INK,
          }}
        >{muted ? '🔇' : '🔊'}</button>
      </div>
      ...
      {feedback && (
        <div className="ss-pop" style={{
          ...
          fontSize: 16, fontWeight: 800, zIndex: 30, pointerEvents: 'none', whiteSpace: 'nowrap',
      ...
      {gameOver && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: VIOLET }}>Game Over!</div>
            {mode === 'classic' ? (
              <>
                <div style={{ fontSize: 13, fontWeight: 600, color: INK_MUTED, textAlign: 'center', lineHeight: 1.5 }}>
                  You reached round {round}<br />
                  Best this session: {Math.max(bestRound, round)}
                </div>
                <div style={{ fontSize: 20 }}>{starRating(Math.max(bestRound, round))}</div>
              </>
            ) : (
              <>
                <div style={{ fontSize: 13, fontWeight: 600, color: INK_MUTED, textAlign: 'center', lineHeight: 1.5 }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: INK, textAlign: 'center' }}>
            ...
            {isT ? (
              <button type="button" onClick={handlePlayAgain} style={{
                ...
                background: `linear-gradient(180deg, ${VIOLET_MID}, ${VIOLET})`, color: '#ffffff',
                fontSize: 13, fontWeight: 800, boxShadow: '0 4px 14px rgba(91,33,182,0.32)',
              }}>Play again</button>
            ) : (
              <div style={{ marginTop: 4, fontSize: 12, fontWeight: 600, color: INK_MUTED }}>
      ...
      {toast && (
        <div className="ss-pop" style={{
          ...
          padding: '7px 18px', color: INK, fontSize: 12.5, fontWeight: 700,
